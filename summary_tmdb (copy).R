...
movies$vote_average <- round(movies$vote_average)

movies$ratio_rev<- movies$revenue - movies$budget
...
output.tree <- ctree(vote_average ~ runtime + ratio_rev + popularity + language_number + dir_experience_mean + pro_experience_mean + act_experience_mean, movies)

plot(output.tree)
predicted<- predict(output.tree)
predicted<- round(predicted)
table(predicted,movies$vote_average)
library(caret)
u <- union(predicted, movies$vote_average)
u
cfm <- confusionMatrix(factor(predicted, u), factor(movies$vote_average, u),  mode = "prec_recall", positive="1")
cfm
...
library(DMwr)
result <- kNN(vote_average ~ runtime + ratio_rev + popularity + language_number + dir_experience_mean + pro_experience_mean + act_experience_mean, movies)
